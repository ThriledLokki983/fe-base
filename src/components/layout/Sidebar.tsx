import { useCallback, useState, useEffect, useRef } from 'react';
import styles from './Sidebar.module.scss';
import {
  LayoutDashboard,
  PlusCircle,
  Zap,
  Users,
  Bot,
  Plug2,
  ChevronDown,
  LogOut,
  Settings,
  User
} from 'lucide-react';

// Menu items configuration with Lucide icons
const menuItems = [
  { id: 'overview', label: 'Overview', icon: <LayoutDashboard size={20} strokeWidth={1.5} /> },
  { id: 'campaigns', label: 'Create campaign', icon: <PlusCircle size={20} strokeWidth={1.5} /> },
  { id: 'automation', label: 'Automation', icon: <Zap size={20} strokeWidth={1.5} /> },
  { id: 'subscriptions', label: 'Subscriptions', icon: <Users size={20} strokeWidth={1.5} /> },
  { id: 'chatbot', label: 'AI Chatbot', icon: <Bot size={20} strokeWidth={1.5} /> },
  { id: 'integrations', label: 'Integrations', icon: <Plug2 size={20} strokeWidth={1.5} /> }
];

const userMenuItems = [
  { id: 'profile', label: 'Profile', icon: <User size={16} strokeWidth={1.5} /> },
  { id: 'settings', label: 'Settings', icon: <Settings size={16} strokeWidth={1.5} /> },
  { id: 'logout', label: 'Logout', icon: <LogOut size={16} strokeWidth={1.5} /> },
];

const Sidebar = () => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const menuRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (menuRef.current && !menuRef.current.contains(event.target as Node)) {
        setIsMenuOpen(false);
      }
    };

    document.addEventListener('mousedown', handleClickOutside);
    return () => document.removeEventListener('mousedown', handleClickOutside);
  }, []);

  const handleMenuClick = useCallback((id: string) => {
    // Handle menu item click
    console.log('Menu clicked:', id);
  }, []);

  const handleUserMenuClick = useCallback((id: string) => {
    // Handle user menu item click
    console.log('User menu clicked:', id);
    setIsMenuOpen(false);
  }, []);

  return (
    <aside className={styles.sidebar}>
      <div className={styles.logo}>
        <h1>Your Logo</h1>
      </div>
      
      <nav className={styles.navigation}>
        <ul className={styles.menu}>
          {menuItems.map(item => (
            <li key={item.id} className={styles.menuItem}>
              <button
                onClick={() => handleMenuClick(item.id)}
                className={styles.menuButton}
              >
                <span className={styles.icon}>{item.icon}</span>
                <span className={styles.label}>{item.label}</span>
              </button>
            </li>
          ))}
        </ul>
      </nav>

      <div className={styles.userProfile} ref={menuRef}>
        <div 
          className={styles.profileButton}
          onClick={() => setIsMenuOpen(!isMenuOpen)}
          role="button"
          tabIndex={0}
          aria-expanded={isMenuOpen}
          aria-haspopup="true"
        >
          <div className={styles.avatarSection}>
            <img 
              src="data:image/jpeg;base64," 
              alt="User avatar" 
              className={styles.avatar}
            />
            <div className={styles.userInfo}>
              <span className={styles.userName}>User Name</span>
              <span className={styles.userEmail}>user@example.com</span>
            </div>
          </div>
          <ChevronDown 
              size={20} 
              strokeWidth={1.5} 
              className={`${styles.chevron} ${isMenuOpen ? styles.chevronRotated : ''}`}
          />
        </div>
        
        {isMenuOpen && (
          <div className={styles.userMenu} role="menu">
            {userMenuItems.map(item => (
              <button
                key={item.id}
                onClick={(e) => {
                  e.stopPropagation();
                  handleUserMenuClick(item.id);
                }}
                className={styles.userMenuItem}
                role="menuitem"
              >
                <span className={styles.menuItemIcon}>{item.icon}</span>
                <span>{item.label}</span>
              </button>
            ))}
          </div>
        )}
      </div>
    </aside>
  );
};


export default Sidebar;
